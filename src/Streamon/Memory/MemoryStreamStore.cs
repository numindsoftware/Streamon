...
public class MemoryStreamStore : IStreamStore
{
    ...
    private static IEnumerable<EventEnvelope> ConvertToEnvelopes(StreamId streamId, BatchId batchId, StreamPosition startingSequence, StreamPosition globalPosition, IEnumerable<object> events, EventMetadata? metadata) =>
        events.Select((e, i) => new EventEnvelope(streamId, EventId.New(), StreamPosition.From(startingSequence.Value + i), StreamPosition.From(globalPosition.Value + i), DateTimeOffset.Now, batchId, e, metadata ?? ExtractMetadata(e)));
