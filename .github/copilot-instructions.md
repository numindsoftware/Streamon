...
```csharp
services.AddStreamSubscription(SubscriptionId.From("my-sub"), StreamSubscriptionType.CatchUp)
...
- `StreamSubscriptionType.LiveOnly` — starts from the current stream end when no checkpoint exists

Subscriptions are registered as **keyed singletons** (`AddKeyedSingleton`) using `SubscriptionId.Value`
as the key, which allows multiple independent subscriptions within one DI container.

### Resolving at Runtime

```csharp
var provisioner = sp.GetRequiredService<IStreamStoreProvisioner>();
var store = await provisioner.CreateStoreAsync("MyTableName");
...
- Multiple subscriptions are supported by keyed DI — always use `AddKeyedSingleton` / `GetRequiredKeyedService` when introducing new subscription-scoped services.
