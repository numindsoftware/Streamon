...
{
    private const string StreamNamespace = "Acme";

    [Fact, Priority(1)]
    public async Task AppendsEventsForProjection()
    {
        var store = await fixture.StreamStoreProvisioner.CreateStoreAsync(StreamNamespace, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var stream1 = await store.FetchEventsAsync(new StreamId("order-1"), cancellationToken: TestContext.Current.CancellationToken);
        var stream2 = await store.FetchEventsAsync(new StreamId("order-2"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        var subscription = await fixture.SubscriptionProvisioner.CreateSubscriptionAsync(SubscriptionId.From("projection-sub"), StreamNamespace, cancellationToken: TestContext.Current.CancellationToken);
        await subscription.PollAsync(cancellationToken: TestContext.Current.CancellationToken);

        var projectionStore = fixture.CreateProjectionStore<OrderProjectionEntity>(StreamNamespace);
        var entity = await projectionStore.ReadAsync(new OrderProjectionEntity { OrderId = "1" }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    {
        var projectionStore = fixture.CreateProjectionStore<OrderProjectionEntity>(StreamNamespace);
        var entity = await projectionStore.ReadAsync(new OrderProjectionEntity { OrderId = "2" }, cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(entity);
        Assert.NotNull(entity.Items);
        Assert.Single(entity.Items);
    ...
    public async Task SkipsUpdateWhenNoInitialStateExists()
    {
        ...
        var projectionStore = fixture.CreateProjectionStore<OrderProjectionEntity>(StreamNamespace);
        var entity = await projectionStore.ReadAsync(new OrderProjectionEntity { OrderId = "3" }, cancellationToken: TestContext.Current.CancellationToken);
        Assert.Null(entity);
    }
    ...
    {
        var projectionStore = fixture.CreateProjectionStore<OrderProjectionEntity>(StreamNamespace);
        var entity1 = await projectionStore.ReadAsync(new OrderProjectionEntity { OrderId = "1" }, cancellationToken: TestContext.Current.CancellationToken);
        var entity2 = await projectionStore.ReadAsync(new OrderProjectionEntity { OrderId = "2" }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ProjectsItemsAddedToOrder()
    {
        ...
        var projectionStore = fixture.CreateProjectionStore<OrderProjectionEntity>(StreamNamespace);
        var entity = await projectionStore.ReadAsync(new OrderProjectionEntity { OrderId = "4" }, cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(entity);
        Assert.NotNull(entity.Items);
